...
struct imguiGizmo
{
    ...
    vg::quat qtV  = vg::quat(1.0f, vg::vec3(0.0f)); // Quaternion value
    vg::quat qtV2 = vg::quat(1.0f, vg::vec3(0.0f)); // Quaternion value
#ifndef IMGUIZMO_USE_ONLY_ROT
    vg::vec3 posPanDolly = vg::vec3(0.f);
    ...
    static ImVector<vg::vec3> sphereVtx;
    static ImVector<int>  sphereTess;
    static ImVector<vg::vec3> cubeVtx;
    static ImVector<vg::vec3> cubeNorm;
    static ImVector<vg::vec3> planeVtx;
    static ImVector<vg::vec3> planeNorm;
    static ImVector<vg::vec3> arrowVtx[4];
    static ImVector<vg::vec3> arrowNorm[4];
    static void buildPlane   (const float size, const float thickness = planeThickness) {
        buildPolygon(vg::vec3(thickness,size,size), planeVtx, planeNorm);
    }
    static void buildCube    (const float size) {
        buildPolygon(vg::vec3(size), cubeVtx, cubeNorm);
    }
    static void buildPolygon (const vg::vec3& size,ImVector<vg::vec3>& vtx,ImVector<vg::vec3>& norm);
    ...
    // helper functions
    ///////////////////////////////////////
    static void resizeAxesOf(const vg::vec3 &newSize) {
...
//      evSuperModifier   -> Super
    ...
    ////////////////////////////////////////////////////////////////////////////
    bool getTransforms(vg::quat& q, const char* label, vg::vec3& dir, float size) {
        float len = length(dir);

        if(len<1.0 && len>= FLT_EPSILON) { normalize(dir); len = 1.0; }
        else if(len< FLT_EPSILON) { dir = vg::vec3(1.f, 0.f, 0.f); len = 1.0; }

        q = angleAxis(acosf(dir.x/len), normalize(vg::vec3(FLT_EPSILON, -dir.z, dir.y)));

        bool ret = drawFunc(label, size);
        if (ret) dir = (q * vg::vec3(1.0f, 0.0f, 0.0f)) * len ; //return vector with original lenght

        return ret;
    }
    // Vec4 (xyz axis, w angle) -> quat -> trackbalTransforms -> quat -> vec4
    ////////////////////////////////////////////////////////////////////////////
    bool getTransforms(vg::quat& q, const char* label, vg::vec4& axis_angle, float size) {
        q = angleAxis(axis_angle.w,vg::vec3(axis_angle)); //g.ConvertFromAxisAngle();

        bool ret = drawFunc(label, size);
        if (ret) axis_angle = vg::vec4(vg::vec3(axis(q)),angle(q));
    ...
    // Axes reduction
    ///////////////////////////////////////
    static vg::vec3 axesResizeFactor;
    static vg::vec3 savedAxesResizeFactor;
...
IMGUI_API bool gizmo3D(const char*, vg::quat&, float=IMGUIZMO_DEF_SIZE, const int=imguiGizmo::mode3Axes|imguiGizmo::cubeAtOrigin);
IMGUI_API bool gizmo3D(const char*, vg::vec4&, float=IMGUIZMO_DEF_SIZE, const int=imguiGizmo::mode3Axes|imguiGizmo::cubeAtOrigin);
IMGUI_API bool gizmo3D(const char*, vg::vec3&, float=IMGUIZMO_DEF_SIZE, const int=imguiGizmo::modeDirection);

IMGUI_API bool gizmo3D(const char*, vg::quat&, vg::quat&, float=IMGUIZMO_DEF_SIZE, const int=imguiGizmo::modeDual|imguiGizmo::cubeAtOrigin);
IMGUI_API bool gizmo3D(const char*, vg::quat&, vg::vec4&, float=IMGUIZMO_DEF_SIZE, const int=imguiGizmo::modeDual|imguiGizmo::cubeAtOrigin);
IMGUI_API bool gizmo3D(const char*, vg::quat&, vg::vec3&, float=IMGUIZMO_DEF_SIZE, const int=imguiGizmo::modeDual|imguiGizmo::cubeAtOrigin);

#ifndef IMGUIZMO_USE_ONLY_ROT

//with Pan & Dolly feature
IMGUI_API bool gizmo3D(const char*, vg::vec3&, vg::quat&, float=IMGUIZMO_DEF_SIZE, const int=imguiGizmo::mode3Axes|imguiGizmo::cubeAtOrigin);
IMGUI_API bool gizmo3D(const char*, vg::vec3&, vg::vec4&, float=IMGUIZMO_DEF_SIZE, const int=imguiGizmo::mode3Axes|imguiGizmo::cubeAtOrigin);
IMGUI_API bool gizmo3D(const char*, vg::vec3&, vg::vec3&, float=IMGUIZMO_DEF_SIZE, const int=imguiGizmo::modeDirection);

IMGUI_API bool gizmo3D(const char*, vg::vec3&, vg::quat&, vg::quat&, float=IMGUIZMO_DEF_SIZE, const int=imguiGizmo::modeDual|imguiGizmo::cubeAtOrigin);
IMGUI_API bool gizmo3D(const char*, vg::vec3&, vg::quat&, vg::vec4&, float=IMGUIZMO_DEF_SIZE, const int=imguiGizmo::modeDual|imguiGizmo::cubeAtOrigin);
IMGUI_API bool gizmo3D(const char*, vg::vec3&, vg::quat&, vg::vec3&, float=IMGUIZMO_DEF_SIZE, const int=imguiGizmo::modeDual|imguiGizmo::cubeAtOrigin);
